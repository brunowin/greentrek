...
export default class MapScreen extends Component {
  ...
  async getDirections(startLoc, destinationLoc) {
    try {
      let respWalk = await fetch(
        `https://maps.googleapis.com/maps/api/directions/json?origin=${startLoc}&destination=${destinationLoc}&mode=walking&key=AIzaSyBoI7_OALZ8qNp2TqbVhoBu5ecVncn-lRI`
      );
      let respBike = await fetch(
        `https://maps.googleapis.com/maps/api/directions/json?origin=${startLoc}&destination=${destinationLoc}&mode=biking&key=AIzaSyBoI7_OALZ8qNp2TqbVhoBu5ecVncn-lRI`
      );
      let respTran = await fetch(
        `https://maps.googleapis.com/maps/api/directions/json?origin=${startLoc}&destination=${destinationLoc}&mode=transit&key=AIzaSyBoI7_OALZ8qNp2TqbVhoBu5ecVncn-lRI`
      );
      ...
      let points = Polyline.decode(
        respJsonWalk.routes[0].overview_polyline.points
      );
    ...
    return (
      <View>
        ...
        >
          ...
          <MilesTime
            transport="Walking"
            transportMiles={this.state.distWalk}
            transportTime={this.state.timeWalk}
          />

          <MilesTime
            transport="Biking"
            transportMiles={this.state.distBike}
            transportTime={this.state.timeBike}
          />

          <MilesTime
            transport="Transit"
            transportMiles={this.state.distTran}
            transportTime={this.state.timeTran}
          />
        </DraggablePanel>
      </View>
    );
